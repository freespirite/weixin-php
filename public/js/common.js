 ...
 * @param input_selector 图片容器
 */
function upload_weixin_image(dialog_title, input_selector, wxid) {
    open_upload_dialog(dialog_title, function (dialog, files) {
        $(input_selector).val(files[0].filepath);
        //$(input_selector + '-preview').attr('src', files[0].preview_url);
        //$(input_selector + '-name').val(files[0].name);
        window.location.reload();
    }, {}, 0, 'image', 'Weixin&wxid='+wxid);
}
function upload_weixin_thumb(dialog_title, input_selector, wxid) {
    open_upload_dialog(dialog_title, function (dialog, files) {
        $(input_selector).val(files[0].filepath);//console.log(files[0]);
